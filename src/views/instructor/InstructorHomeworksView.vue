    ...
    <div v-else class="review-content">
      ...
      <div class="homework-info-card">
        <div class="info-section">
          <h2>{{ homework.lesson?.title || 'Без названия' }}</h2>
          <div class="info-grid">
            <div class="info-item">
              <strong>Студент:</strong>
              <span>{{ homework.user?.username || 'Неизвестно' }}</span>
            </div>
            <div class="info-item">
              <strong>Курс:</strong>
              <span>{{ homework.lesson?.course?.title || 'Без курса' }}</span>
            </div>
            <div class="info-item">
...
const loadHomework = async () => {
  try {
    ...
    if (foundHomework) {
      ...
      console.log('📋 Все доступные ДЗ:', instructorStore.allHomeworks)
    }
  } catch (err: any) {
