...
import type { Lesson, LessonDetail } from '@/types/api' // Добавляем импорт типов

export const lessonService = {
  // Получить уроки курса
  async getCourseLessons(courseId: number): Promise<{ lessons: Lesson[] }> {
    // Изменяем возвращаемый тип
    try {
      const response = await apiClient.get(`/courses/${courseId}/lessons/`)
      return response.data // Возвращаем весь объект ответа
    } catch (error) {
